...
from scripts.colision import colision_plataforma
...
class PlayerOne(Personaje):
    ...
    def cargar_imagenes(self):
        ...
        for cuadro in self.cuadros_correr_d:
            self.cuadros_correr_i.append(
                pg.transform.flip(cuadro, True, False))

        self.cuadro_saltar_d = self.game.spritesheet.get_imagen(0, 396, 64, 64)
        self.cuadro_saltar_i = pg.transform.flip(
            self.cuadro_saltar_d, True, False)
    ...
    def animar(self):
        ...
        if self.caminando:
            if este_instante - self.ultimo_update > ACTUALIZACION_CUADROS:
                ...
                self.cuadro_actual = (
                    self.cuadro_actual + 1) % len(self.cuadros_correr_d)
                if self.vel.x > 0:
        ...
        if not self.saltando and not self.caminando:
            if (este_instante - self.ultimo_update) > ACTUALIZACION_CUADROS:
                self.ultimo_update = este_instante
                self.cuadro_actual = (
                    self.cuadro_actual + 1) % len(self.cuadros_idle)
...
class Botaraña(Personaje):
    ...
    def transformar(self, imagen):
        ...
        self.image = pg.transform.scale(
            imagen, (TAMAÑO_TILE_BOTARAÑA, TAMAÑO_TILE_BOTARAÑA))

    def cargar_imagenes(self):
        self.cuadros_idle = [self.game.spritesheet_araña.get_imagen(132, 396, 64, 64),
                             self.game.spritesheet_araña.get_imagen(
                                 132, 264, 64, 64),
                             self.game.spritesheet_araña.get_imagen(
                                 132, 330, 64, 64),
        ...
        self.cuadro_muerte = self.game.spritesheet_araña.get_imagen(
            132, 66, 64, 64)
